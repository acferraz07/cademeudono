...
export default function DevolvidosPage() {
  ...
  useEffect(() => {
    if (!token) return
    announcementsApi
      .findAll(token, { status: 'RETURNED_TO_OWNER', limit: 50 })
  ...
  return (
    <div>
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-900">🌟 Pets devolvidos ao tutor ✅</h1>
        <p className="text-sm text-gray-500 mt-1">
          Prova do impacto real do Cadê Meu Dono — {items.length} histórias de final feliz
        </p>
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-20 text-gray-400">
          ...
          <p className="font-medium text-gray-600">Nenhum pet devolvido registrado ainda</p>
          <p className="text-sm mt-2">Quando um pet perdido for encontrado, aparecerá aqui!</p>
        </div>
      ) : (
